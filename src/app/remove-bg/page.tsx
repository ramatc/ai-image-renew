...
import { createRemovePrediction, getRemovePrediction } from "@/actions";
...
export default function PageRemove() {
  ...
  async function handleSubmit(_state: null | Prediction, formData: FormData) {
    let prediction = await createRemovePrediction(formData);

    while (["starting", "processing"].includes(prediction.status)) {
      prediction = await getRemovePrediction(prediction.id);
